...
function isVowel(value){
    if(value === 'e' || value ===  'a' || value ===  'i' || value === 'u' || value === "o"){
        return true
    }else if(value === 'E'|| value ===  'A' || value ===  'I' || value ===  'U'|| value ===  'O'){
        return true
    }else{
        return false
    }
...
function hasVowels(value){
    var stringVowels = 'aeiouAEIOU'
    for(var i = 0; i < value.length; i++){
        if(stringVowels.indexOf(value[i]) !== -1){
            return true
        }
    }
    return false
...
function removeVowels(string) {
    return string.replace(/[aeiouAEIOU]/ig,'');
}
...
function startsWithVowel(string){
    return ('aeiouAEIOU'.indexOf(string[0]) !== -1)
}
...
function endsWithVowel(string) {
    return ('aeiouAEIOU'.indexOf(string[string.length - 1]) !== -1);
}
...
function firstToLast(sequence) {
    var shift = sequence.shift();
    sequence[sequence.length] = shift;
    return sequence;
}
